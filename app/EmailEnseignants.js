...
import { useTeachers } from '../hooks/EmailHooks';
import CheckBox from 'expo-checkbox';

const EmailTeachersPage = () => {
  const { 
    teachers, 
    ...
    toggleSelectAll: toggleSelectAllTeachers,
    toggleSelectTeacher 
  } = useTeachers();
  ...
  // Fonction pour récupérer le mot de passe d'un enseignant depuis la base de données
  const fetchUserPassword = async (userId) => {
    try {
      const endpoint = `/api/Teachers/${userId}/password`;
  ...
  // Fonction pour envoyer un email direct avec les informations de connexion de l'enseignant
  ...
  // Fonction pour envoyer des emails à tous les enseignants sélectionnés
  const sendMassEmail = async () => {
    ...
    try {
      let successCount = 0;
      let errors = [];
      const selectedUserDetails = teachers.filter(user => 
        selectedTeachers.includes(user.id)
      );
      ...
      if (successCount === selectedUserDetails.length) {
        Alert.alert(
          "Succès",
          `Emails envoyés avec succès à ${successCount} enseignant(s)`,
          [{ text: "OK" }]
        );
      } else {
        Alert.alert(
  ...
  const renderUserItem = ({ item }) => (
    <View style={[
      styles.tableRow,
      selectedTeachers.includes(item.id) && styles.selectedRow
    ]}>
      <View style={styles.checkboxCell}>
        <CheckBox
          value={selectedTeachers.includes(item.id)}
          onValueChange={() => toggleSelectTeacher(item.id)}
          color={selectedTeachers.includes(item.id) ? '#8B2FC9' : undefined}
        />
      </View>
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorText}>
            Erreur lors du chargement des enseignants: {teachersError}
          </Text>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Emails Enseignants</Text>
        ...
        <View style={styles.toolbarContainer}>
          <TouchableOpacity
            style={[
              ...
              (selectedTeachers.length === 0 || massEmailSending) && styles.disabledButton
            ]}
            onPress={sendMassEmail}
            disabled={selectedTeachers.length === 0 || massEmailSending}
          >
            {massEmailSending ? (
            ...
            <Text style={styles.massEmailButtonText}>
              {massEmailSending 
                ? 'Envoi en cours...' 
                : `Envoyer aux sélectionnés (${selectedTeachers.length})`
              }
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.tableContainer}>
          <View style={styles.tableHeader}>
            <View style={styles.checkboxHeaderCell}>
              <CheckBox
                value={selectAllTeachers}
                onValueChange={toggleSelectAllTeachers}
                color={selectAllTeachers ? '#6C5CE7' : undefined}
              />
          ...
          <View style={styles.paginationContainer}>
            <Text style={styles.paginationInfo}>
              {filteredData.length} enseignant(s) affiché(s)
            </Text>
...
if (!isLoggedIn) {
  ...
  const sendDirectEmail = async (user) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        throw new Error(result.message || t('emailSendError'));
      }
    } catch (error) {
      Alert.alert(
        t('error'),
        `${t('emailSendError')} ${user.prenom} ${user.nom}: ${error.message}`,
        [{ text: t('ok') }]
      );
    } finally {
      setDirectEmailSending(false);
  ...
  const sendMassEmail = async () => {
    ...
    try {
      ...
      if (successCount === selectedUserDetails.length) {
        Alert.alert(
          t('success'),
          `${t('emailsSentSuccess')} ${successCount} ${t('teachersDisplayed')}`,
          [{ text: t('ok') }]
        );
      } else {
        Alert.alert(
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          <Ionicons name="warning-outline" size={48} color="#e74c3c" />
          <Text style={styles.errorText}>
            {t('errorLoadingTeachers')} {teachersError}
          </Text>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <StatusBar barStyle="dark-content" backgroundColor="#FFF" />
      <View style={styles.header}>
        {/*<Text style={styles.headerTitle}>{t('emailTeachers')}</Text>
        <View style={styles.underline} />*/}
        {/* Barre de recherche hors de la table */}
        <View style={styles.searchContainer}>
          <Ionicons name="search-outline" size={20} color="#999" style={styles.searchIcon} />
      ...
      <View style={styles.contentContainer}>
        <View style={styles.toolbarContainer}>
          ...
          >
            ...
            <Text style={styles.massEmailButtonText}>
              {massEmailSending 
                ? t('sending') 
                : `${t('sendToSelected')} (${selectedTeachers.length})`
              }
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.tableContainer}>
          ...
          <View style={styles.paginationContainer}>
            <Text style={styles.paginationInfo}>
              {filteredData.length} {t('teachersDisplayed')}
            </Text>
            <Text style={styles.selectedInfo}>
              {selectedTeachers.length} {t('selected')}
            </Text>
          </View>
        </View>
      </View>
...
actionCell: {
  flex: 1,
  flexDirection: 'row',
  justifyContent: 'center', // Changé de 'flex-start' vers 'center'
  alignItems: 'center',
  paddingLeft: 4, // Ajouté pour décaler l'icône vers la droite
},
  idCell: {
    width: 50,
    textAlign: 'center',
...
export default EmailTeachersPage;