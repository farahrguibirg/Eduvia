...
export default function Exercice() {
  ...
  const renderForm = () => (
    <View style={styles.formContainer}>
      <Text style={styles.subtitle}>Générer un QCM</Text>
      ...
      <View style={styles.fileContainer}>
        ...
        <TouchableOpacity style={styles.fileButton} onPress={handleFileSelection}>
          ...
          <Text style={styles.fileButtonText}>
            {selectedFile ? "Changer de fichier" : "Sélectionner un fichier PDF"}
          </Text>
        </TouchableOpacity>
            ...
            <View style={styles.fileInfoContainer}>
              ...
              <View style={styles.fileDetails}>
                ...
                <Text style={styles.fileSize}>
                  {selectedFile.size ? `${(selectedFile.size / 1024).toFixed(1)} KB` : "Taille inconnue"}
                </Text>
              </View>
            </View>
      ...
      >
        {loading ? (
          ...
          <>
            ...
            <Text style={styles.submitButtonText}>Générer le QCM</Text>
          </>
        )}
      </TouchableOpacity>
    </View>
  ...
  const renderQCM = () => (
    <ScrollView style={styles.qcmContainer}>
          ...
          <View style={styles.reponsesList}>
            {question.reponses.map((reponse) => (
              <View key={reponse.id} style={styles.reponseItem}>
                <View style={[
                  styles.reponseIndicator, 
                  reponse.est_correcte ? styles.correctReponse : {}
                ]} />
                <Text style={styles.reponseText}>{reponse.texte}</Text>
              </View>
            ))}
          </View>
        </View>
      ...
      >
        ...
        <Text style={styles.backButtonText}>Nouveau QCM</Text>
      </TouchableOpacity>
    </ScrollView>
  );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Générateur de QCM</Text>
    ...
    try {
      ...
      if (exerciceDetail && exerciceDetail.questions && exerciceDetail.questions.length > 0) {
        ...
        if (exerciceDetail === undefined || exerciceDetail === null) {
          ...
          console.error('Exercice incomplet:', exerciceDetail);
        }
        Alert.alert('Erreur', 'Impossible de charger les détails de l\'exercice');
        return undefined;
      }
    } catch (error) {
      if (error) {
          ...
          <View key={question.id} style={styles.questionContainer}>
            ...
            <View style={styles.reponsesList}>
              ...
              <Text style={{ fontStyle: 'italic', marginTop: 4 }}>
                *Réponse : {String.fromCharCode(65 + correctIndex)}*
              </Text>
            </View>
          </View>
        );
...
const styles = StyleSheet.create({
  ...
  header: {
    flexDirection: 'row',
    justifyContent: 'flex-start', // Changé de space-between à flex-start pour aligner à gauche
    alignItems: 'center',
    padding: 16,
    backgroundColor: '#fff',
    borderBottomWidth: 1,
  ...
  return (
    ...
    >
      <View style={styles.introContent}>
        ...
        <Text style={styles.introTitle}>Je suis votre assistant IA, prêt à générer des QCM à partir de vos documents.</Text>
        <Text style={styles.introSubtitle}>
        Téléversez un fichier quand vous êtes prêt, et je m’occupe du reste !"
        </Text>
...
export default function Exercice() {
  ...
  {/*const handleGenerateQuiz = async () => {
    try {
      ...
      if (isNaN(chrono) || chrono < 1 || chrono > 60) {
        Alert.alert('Erreur', 'La durée doit être un nombre entre 1 et 60 minutes');
        return;
      }

      const response = await fetch(`${API_URL}/quiz/generate-from-qcm/${selectedQcmId}`, {
        method: 'POST',
        headers: {
      ...
      Alert.alert('Erreur', error.message || 'Erreur lors de la génération du quiz');
    }
  };*/}
  // Fonction pour gérer la génération du quiz
  {/*
  const handleGenerateQuiz = async () => {
    try {
      ...
      // Récupérer l'ID étudiant depuis le stockage local ou le contexte d'authentification
      // Pour cet exemple, nous allons utiliser une valeur fixe, mais vous devriez idéalement
      // récupérer l'ID de l'étudiant connecté à partir du système d'authentification
      const etudiantId = 1; // À remplacer par la vraie source d'ID étudiant
  ...
  const renderQCM = () => (
    <ScrollView style={styles.qcmContainer}>
      <View style={styles.qcmHeader}>
        ...
        >
          <Ionicons name="create-outline" size={20} color="#FFF" />
          <Text style={styles.generateQuizButtonText}>Générer un quiz</Text>
        </TouchableOpacity>
      </View>
          ...
          {isSidebarOpen && (
            ...
            {histories && histories.map((qcm) => (
                  <View key={qcm.id} style={{ marginBottom: 12, backgroundColor: '#fff', borderRadius: 8, padding: 12, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 }}>
                    ...
                    <Text style={{ color: '#888', fontSize: 13 }}>Créé le : {qcm.date_creation}</Text>
                    <TouchableOpacity
                    ...
                    >
                      <Text style={{ color: 'white', fontWeight: 'bold' }}>Générer un quiz</Text>
                    </TouchableOpacity>
                  </View>
                ))}
              </SidebarExercice>
          ...
          {showDurationModal && (
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                <Text style={styles.modalTitle}>Durée du quiz</Text>
                <Text style={styles.modalSubtitle}>Entrez la durée en minutes (1-60)</Text>
                <TextInput
      ...
      {showIntro ? (
        ...
        <>
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.title}>{t('qcmGenerator')}</Text>
          </View>
          ...
          {showDurationModal && (
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                <Text style={styles.modalTitle}>{t('quizDuration')}</Text>
...
const styles = StyleSheet.create({
  ...
  header: {
    flexDirection: 'row',
    justifyContent:  'space-between', // Changé de space-between à flex-start pour aligner à gauche
    alignItems: 'center',
    padding: 16,
    backgroundColor: '#fff',
    borderBottomWidth: 1,
